import type { EfetivoAdministrativoRow, EfetivoRow } from './types'
...
const findAdmHeaderRowIndex = (values: string[][]) => {
  const nomeMarkers = ['NOME']
  const secaoMarkers = ['SECAO', 'SEÇÃO']

  const nomeSet = new Set(nomeMarkers.map(normalizeHeader))
  const secaoSet = new Set(secaoMarkers.map(normalizeHeader))

  const maxScan = Math.min(values.length, 30)
  for (let i = 0; i < maxScan; i++) {
    const row = values[i] ?? []
    const normalizedCells = row.map((c) => normalizeHeader((c ?? '').toString()))
    const hasNome = normalizedCells.some((c) => nomeSet.has(c))
    const hasSecao = normalizedCells.some((c) => secaoSet.has(c))
    if (hasNome && hasSecao) return i
  }
...
const parseEfetivoAdministrativoRowsFromTable = (values: string[][]): EfetivoAdministrativoRow[] => {
  if (values.length === 0) return []

  const headerRowIndex = findAdmHeaderRowIndex(values)
  const headerRow = values[headerRowIndex] ?? []
  const dataRows = values.slice(headerRowIndex + 1)
  const headers = (headerRow ?? []).map((v) => (v ?? '').toString())
...
export const fetchEfetivoAdministrativoRowsFromSheets = async (
  config: SheetsFetchConfig,
): Promise<EfetivoAdministrativoRow[]> => {
  const url = new URL(
    `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(config.spreadsheetId)}/values/${encodeURIComponent(
      config.rangeA1,
    )}`,
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => '')
    throw new Error(`Falha ao carregar Google Sheets (ADM) (${res.status}). ${body}`)
  }

  const json = (await res.json()) as SheetsValuesResponse
  const values = json.values ?? []
  return parseEfetivoAdministrativoRowsFromTable(values)
}
